...
export default () => {
  ...
  const executeCommand = lastWord => {
    ...
    // COMMAND TEMPLATES
    const createCommand = ({ fn, replaceStr = '', go, replaceSpace, skipReplace }) => () => {
      if (!skipReplace) replace(`${replaceSpace ? ' ' : ''}${lastWord}`, replaceStr);
      if (fn) fn();
    ...
    const commands = {
      ...
      ol: createCommand({
        fn: () => {
          ...
          const validLine = line => line.trim() !== '' && line.trim() !== lastWord;
          const isOrderedList = lines.filter(validLine).every(line => regex.test(line));
          let index = 1;
    ...
    if (key === SPACEBAR_KEY) {
      ...
      if (key === '∂') {
        ...
        const swapLines = async direction => {
          ...
          contentArray[index] = contentArray.splice(swapIndex, 1, contentArray[index])[0];
          await setContent(contentArray.join('\n'));
          // set cursor
          const newStart =
            contentArray.reduce((total, line, idx) => (idx <= swapIndex - 1 ? (total += line.length + 1) : total), 0) +
            offset;
